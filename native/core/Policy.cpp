...
namespace mterm {
namespace {
const QSet<QString> reads{"filesystem.read", "git.read",       "process.read",
                          "system.info",     "workspace.read", "task.read"};
...
void Policy::setWorkspace(QString workspace) {
    workspace_ = std::move(workspace);
    developer_ = false;
    revoke();
}
void Policy::setDeveloper(bool enabled) {
    if (developer_ != enabled)
        revoke();
    developer_ = enabled;
}
bool Policy::allows(const QString &capability, qint64 nowMs) const {
    if (workspace_.isEmpty() || nowMs < 0)
        return false;
    if (reads.contains(capability))
        return true;
    if (!developer_)
        return false;
    if (writes.contains(capability))
        return true;
...
bool Policy::grant(const QString &capability, qint64 nowMs, qint64 durationMs) {
    ...
    if (!developer_ || workspace_.isEmpty() || !approvals.contains(capability) || nowMs < 0 ||
        durationMs <= 0 || durationMs > maxDuration ||
        nowMs > std::numeric_limits<qint64>::max() - durationMs)
        return false;
    grants_[capability] = nowMs + durationMs;
    return true;
}
void Policy::revoke() {
    grants_.clear();
