...
void rejectAliases(const QString &path) {
    ...
    if (info.isSymLink())
        fail("Symbolic links are not permitted in native file operations");
#ifdef Q_OS_WIN
    ...
    if (info.isFile()) {
        ...
        if (handle == INVALID_HANDLE_VALUE)
            fail("Cannot verify file identity");
        BY_HANDLE_FILE_INFORMATION identity{};
        const bool ok = GetFileInformationByHandle(handle, &identity) != 0;
        CloseHandle(handle);
        if (!ok || identity.nNumberOfLinks > 1)
            fail("Hard-linked or unverifiable file identity");
    }
#else
    struct stat identity{};
    if (::lstat(QFile::encodeName(path).constData(), &identity) == 0 && S_ISREG(identity.st_mode) &&
        identity.st_nlink > 1)
...
FileService::FileService(QString root) {
    ...
    if (root_.isEmpty())
        fail("Cannot canonicalize workspace");
}
QString FileService::resolve(const QString &relative, bool writing) const {
    if (relative.isEmpty() || relative.size() > 4096 || relative.contains(QChar::Null))
    ...
    if (QDir::isAbsolutePath(normalized) || normalized.startsWith('/') || normalized.contains(':'))
        fail("Absolute/drive/stream paths are forbidden");
    const auto parts = normalized.split('/');
    static const QRegularExpression device("^(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])$",
                                           QRegularExpression::CaseInsensitiveOption);
    QString candidate = root_;
    for (qsizetype i = 0; i < parts.size(); ++i) {
        const auto &part = parts[i];
        const auto lower = part.toLower();
        if (part.isEmpty() || part == "." || part == ".." || part.endsWith('.') ||
            part.endsWith(' ') || device.match(part.section('.', 0, 0)).hasMatch())
            fail("Ambiguous or reserved path component");
        if (lower == ".git" || lower == ".ssh" || lower == ".gnupg" || lower == ".env" ||
            lower.startsWith(".env."))
            fail("Protected secret or repository metadata path");
        candidate = QDir(candidate).filePath(part);
        rejectAliases(candidate);
        const QFileInfo info(candidate);
        if (!info.exists()) {
            if (writing && i == parts.size() - 1)
                return candidate;
...
#endif
        if (canonical.compare(root_, sensitivity) != 0 &&
            !canonical.startsWith(root_.endsWith('/') ? root_ : root_ + '/', sensitivity))
            fail("Path escapes the workspace");
        if (i < parts.size() - 1 && !info.isDir())
...
FileSnapshot FileService::read(const QString &relative) const {
    ...
    if (!info.isFile() || info.size() > MaxBytes)
        fail("File exceeds the native text-size limit");
    QFile file(path);
    if (!file.open(QIODevice::ReadOnly))
        fail("Cannot open workspace file");
    const auto bytes = file.read(MaxBytes + 1);
    if (bytes.size() > MaxBytes || file.error() != QFileDevice::NoError)
        fail("File changed size or failed while reading");
    if (bytes.contains('\0'))
        fail("Binary file is not a text document");
...
FileSnapshot FileService::write(const QString &relative, const QString &text,
                                const QString &expectedVersion) const {
    const auto bytes = text.toUtf8();
    if (bytes.size() > MaxBytes || bytes.contains('\0') || expectedVersion.isEmpty())
        fail("Invalid file content/version");
    const auto path = resolve(relative, true);
    const auto verify = [&] {
        if (expectedVersion == "missing") {
            if (QFileInfo::exists(path))
                fail("STALE_WRITE: target now exists");
        } else if (read(relative).version != expectedVersion)
            fail("STALE_WRITE: file changed since opening");
    };
    verify();
    QSaveFile file(path);
    ...
    // Recheck immediately before replacement; external writers still require a future
    // handle-anchored protocol.
    resolve(relative, true);
    verify();
    if (!file.commit())
        fail("Atomic file commit failed");
    return {relative, text, digest(bytes)};
}
} // namespace mterm
