...
ProcessRunner::ProcessRunner(QObject *parent) : QObject(parent) {
    ...
    connect(&process_, &QProcess::readyReadStandardOutput, this, [this] {
        const auto b = process_.readAllStandardOutput();
        output_.append(b);
        emit outputReady(b, false);
    });
    connect(&process_, &QProcess::readyReadStandardError, this, [this] {
        const auto b = process_.readAllStandardError();
        errors_.append(b);
        emit outputReady(b, true);
    });
...
#ifdef Q_OS_WIN
        HANDLE child =
            OpenProcess(PROCESS_SET_QUOTA | PROCESS_TERMINATE, FALSE, DWORD(process_.processId()));
        const bool owned = child && AssignProcessToJobObject(static_cast<HANDLE>(job_), child);
        if (child)
            CloseHandle(child);
        if (!owned) {
            error_ = "Cannot establish owned process job";
            process_.kill();
            return;
        }
#endif
...
bool ProcessRunner::start(const Command &command) {
    if (active_ || command.program.isEmpty() || command.program.contains(QChar::Null) ||
        !QDir(command.cwd).exists() || command.timeoutMs < 1 || command.timeoutMs > 3600000 ||
        command.input.size() > 1000000)
        return false;
    for (const auto &arg : command.arguments)
        if (arg.contains(QChar::Null))
            return false;
#ifdef Q_OS_WIN
    ...
    active_ = true;
    elapsed_.start();
    process_.setWorkingDirectory(command.cwd);
    process_.setProgram(command.program);
    process_.setArguments(command.arguments);
    // Queue launch so even synchronous Windows failures cannot outrun the accepted-run response.
    QTimer::singleShot(0, this, [this, command] {
        ...
        if (!command.input.isEmpty())
            process_.write(command.input);
        process_.closeWriteChannel();
        timer_.start(command.timeoutMs);
    });
    return true;
}
...
void ProcessRunner::finish(int code) {
    ...
    if (!out.isEmpty()) {
        output_.append(out);
        emit outputReady(out, false);
    }
    if (!err.isEmpty()) {
        errors_.append(err);
        emit outputReady(err, true);
    }
    closeJob();
    active_ = false;
    emit completed({code, cancelled_, timedOut_, error_, output_.bytes(), errors_.bytes(),
                    output_.droppedBytes() + errors_.droppedBytes(), elapsed_.elapsed()});
}
} // namespace mterm
