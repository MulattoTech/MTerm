...
QJsonObject parse(const QByteArray &line) {
    ...
    if (e.error != QJsonParseError::NoError || !doc.isObject())
        throw std::runtime_error("Invalid JSONL provider event");
    return doc.object();
}
} // namespace
QList<QJsonObject> JsonLines::feed(const QByteArray &chunk) {
    ...
    while (pos < chunk.size()) {
        ...
        const auto count = end < 0 ? chunk.size() - pos : end - pos;
        if (pending_.size() + count > maxLine) {
            pending_.clear();
            throw std::runtime_error("JSONL event exceeds 1 MiB");
        }
        pending_.append(chunk.constData() + pos, count);
        if (end < 0)
            break;
        if (!pending_.trimmed().isEmpty())
            result.append(parse(pending_));
        pending_.clear();
        pos = end + 1;
        if (result.size() > 4096)
            throw std::runtime_error("JSONL event burst exceeds limit");
    }
    return result;
}
QList<QJsonObject> JsonLines::finish() {
    auto last = std::move(pending_);
    pending_.clear();
